...
<VContainer>
  ...
  <footer class="flex flex-row gap-4 items-center flex-wrap text-stone-600">
    ...
    <VSelect options={supportedLocales.map(l => ({ value: l.id, title: l.title }))} value={$locale ?? 'en'}>
      {#snippet children()}
        <span class="inline-flex i-bx:world"></span>
        <span>{ supportedLocales.find(l => l.id === $locale)?.title ?? supportedLocales.find(l => l.id === 'en')!.title }</span>
      {/snippet}
      {#snippet optionSnippet(option, select)}
        <a onclick={() => select(option.value)} rel="alternate" hreflang={option.value} class="w-full text-left" href="/{option.value}/">{ option.title }</a>
      {/snippet}
    </VSelect>
  </footer>
